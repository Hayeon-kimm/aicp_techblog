---
layout: post
title: "[09-04-2026] 사이버 노예 AI Agent 만들기 - (3) n8n Discord 요약 봇 구축 및 봇 간 통신 문제 해결"
date: 2026-04-09 18:30:00 +0900
categories: [AI Agent]
tags: [n8n, discord-bot, claude-api, bot-to-bot, allowed-mentions]
author: Hwijun Kim
description: "n8n 기반 Discord 요약 봇 워크플로우를 구축하고, 봇 간 통신 문제를 해결한 과정을 정리해요."
---

## 요약

n8n 기반 Discord 요약 봇 워크플로우를 구축했어요. Webhook → 메시지 수집 → JS 필터링 → Claude API 호출 → Discord 전송 파이프라인을 완성했고, 봇→봇 메시지 허용 및 멘션 파싱 문제를 해결했어요.
...
- n8n 기반 Discord 요약 봇 워크플로우를 구축했어요 (Webhook → 메시지 수집 → JS 필터링 → Claude API 호출 → Discord 전송).
...
- Discord 봇(`server.js`)에 파일 수신/전송 기능을 추가했어요 (텍스트 파일은 프롬프트에 삽입, 이미지는 `--file` 플래그로 전달).
